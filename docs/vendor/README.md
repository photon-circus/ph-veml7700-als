# Vendor source record

Do not commit vendor PDFs unless redistribution terms explicitly permit it.
Pin, hash, and review local copies instead.

`docs/vendor/*.pdf` is intentionally ignored by Git, excluded from exported
archives, and rejected by pack validation if tracked. Changing those guardrails
requires prior owner documentation of permissive redistribution rights.

## Required sources

- Vishay VEML7700 datasheet, document 84286, Rev. 1.8, 28-Nov-2024:
  https://www.vishay.com/docs/84286/veml7700.pdf
- Vishay application note “Designing the VEML7700 Into an Application,”
  document 84323, Revision 06-Mar-2025:
  https://www.vishay.com/docs/84323/designingveml7700.pdf
- Product page:
  https://www.vishay.com/en/product/84286/

## Owner record to complete

- [ ] retrieval date recorded;
- [ ] local PDF SHA-256 recorded;
- [ ] every hardware-contract verification box reviewed;
- [ ] discrepancies between prose and register tables logged in `DECISIONS.md`;
- [ ] any later source revision reviewed before release.

## Local observations

These observations do not check any owner-verification box:

- on 2026-08-08, a local untracked copy of document 84323 was present with
  SHA-256 `422f2bea390e145d0d082f40fdeaad4945c79beec159d6600d4007da0aaed558`;
- no local copy of document 84286 was present;
- the retrieval date and redistribution permission for the local document have
  not been established, so the PDF remains outside the pack inventory.
