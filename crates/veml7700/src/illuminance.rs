...
impl NominalScale {
    /// Construct the nominal scale recorded by `S-26`.
    ...
    /// Scale an ALS count using the nominal `S-26` ratio.
    pub const fn scale_counts(self, counts: u16) -> MicroLux {
        MicroLux::from_micro_lux(counts as u64 * self.micro_lux_per_count as u64)
    }

    /// Nominal illuminance produced by the scale at maximum ADC code.
    ///
    /// This is a numerical property of the nominal `S-26` scale, not a physical
    /// illuminance bound. At `u16::MAX`, the driver assumes neither clipping nor
    /// a scene lower bound (`S-51`, `S-52`). See
    /// [`AlsCounts::is_max_code`](crate::AlsCounts::is_max_code) for the exact
    /// maximum-code observation.
